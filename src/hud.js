  ...
  getConstructionChecklistLines(state) {
    ...
    if (mode === "detailed") {
      ...
      if (state.advancedOps) {
        lines.push(
          "OPS LB:" +
            String(state.advancedOps.labProduct || state.advancedOps.labStatus || "idle").toUpperCase() +
            " FC:" +
            String(state.advancedOps.factoryProduct || state.advancedOps.factoryStatus || "idle").toUpperCase() +
            " TK:" +
            String(state.advancedOps.taskLabel || "idle").toUpperCase(),
        );
      }
      lines.push(
        "ROADS " +
