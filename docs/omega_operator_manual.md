...
| `ops.empire(section)` | Show empire summary and owned-room overview, logistics pressure rollup, or labor coverage plus RCL8 GCL push demand rollup. | `section`: optional; use `logistics` for pressure or `labor` for labor/GCL push demand state. | `ops.empire("labor")` | Empire report object, or section object with lines and rollup. | Updates progress/logistics history in generated reports. Logistics and labor rollups do not create logistics requests or execute hauler, market, terminal, spawn, factory, lab, or boost actions. | Read Only | `src/ops.js`, `src/room_reporting.js` |
...
- `ops.room("ROOM", "labor")` and `ops.empire("labor")`: read-only worker labor coverage diagnostics for current/desired labor, deficits, spawn state, pending worker intents, low-energy/busy-spawn/priority blockers, repeated `restore worker labor coverage` rooms, and RCL8 GCL push eligibility, upgrader demand, body/profile, and blockers.
